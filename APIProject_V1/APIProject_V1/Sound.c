...
BOOL soundPlayLoopingWav(SoundInfo* sound)
{
	...
	dwReturn = mciSendCommand(sound->DeviceID, MCI_STATUS, MCI_STATUS_ITEM, (DWORD)(LPVOID)&mciStatusParams);
	if (dwReturn != 0)
	{
#ifdef _LOGGER_H
		logDebugMCIError(dwReturn, "soundPlayLoopingWav() getLength");
#endif // _LOGGER_H
		soundClose(sound);
		return FALSE;
	}
	...
	{
#ifdef _LOGGER_H
		logDebugMCIError(dwReturn, "soundPlayLoopingWav() getPostion");
#endif // _LOGGER_H
		soundClose(sound);
		return FALSE;
	}
	...
	if (position == 0)
	{
		...
		{
#ifdef _LOGGER_H
			logDebugMCIError(dwReturn, "soundPlayLoopingWav()");
#endif // _LOGGER_H
			soundClose(sound);
			return FALSE;
		}
